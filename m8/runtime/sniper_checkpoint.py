...
def resolve_streaming_checkpoint_path(session_id: Optional[str] = None) -> str:
    sid = str(session_id or pipeline_session_id() or "").strip()
    safe = re.sub(r"[^a-zA-Z0-9._-]", "_", sid)[:96] or "unknown"
    return f"data/tmp/m8_sniper_streaming_checkpoint_{safe}.json"
