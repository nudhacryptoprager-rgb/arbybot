...
| **arbitrum_one** | camelot_v3 | algebra | 0x1a3c… | quoter: 0x0Fc7… | dynamic | YES | CANDIDATE | Included in `onboard_arbitrum_one_candidate.yaml`; online verification pending |
...
| **scroll** | nuri_v3 | uniswap_v3 | 0xAAA3… | quoter_v2: 0xAAAE… | 100,500,3000,10000 | YES | RE-ENABLED (R27) | Re-enabled in `coverage_intent_scroll.yaml`; online verification pending |
...
1. **scroll/nuri_v3**: ~~`coverage_intent_scroll.yaml` excluded nuri_v3 as algebra-incompatible~~ — **RESOLVED R27**. Trust anchor (`dexes.yaml`) declares `adapter_type: uniswap_v3` with `quoter_v2`. Config now aligned; `require_cross_dex: true`. Online verification pending (`onboard_scroll_stage1.yaml`).
2. **arbitrum_one/camelot_v3**: Adapter ready (algebra/PRODUCTION), included in `onboard_arbitrum_one_candidate.yaml` for additive candidate testing. Online verification pending.
...
| 6 | **scroll** | monitoring_only | nuri_v3 re-enabled (R27), online proof pending; ecosystem maturity |
