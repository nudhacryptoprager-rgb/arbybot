...
**Cross-Mechanic Sniper Edge (гілка):** Hot-path token-neighborhood **code path implemented** (`candidate_tokens_from_event`, non-anchor `T-X` events, `selected_focus_token`); **full M8 event token-first runtime acceptance pending** (`ready_for_bridge_shadow` still false on last smoke). Batch rolling default `--expansion-mode token_neighborhood`. Blocker: `M8_2_TOKEN_NEIGHBORHOOD_EXPANSION_MISSING` until live 15m WS shows `subgraph_ready` + fresh rolling expansion without legacy `ADAPTER_RESOLVE_PENDING`. M9 shadow лише після `acceptance.ready_for_bridge_shadow=true` + `ARBY_M9_CYCLE_LENGTHS=2,3,4`.
