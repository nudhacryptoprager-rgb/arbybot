...
**Status**: **BLOCKED** — blockers **`CYCLE_LANE_SIZING_AND_LIQUIDITY_MISMATCH`**, **`UNKNOWN_TOKEN_SYMBOL_RESOLUTION`** (post-decimals). Decimals truth layer landed: `token*_decimals_source`, on-chain bridge enrichment, M8 sniper `batch_decimals`, topology_probe `decimals_status=UNKNOWN_DIAGNOSTIC`. **Admission A/B post-decimals** (`diagnostic_soft`): production `edges_built=4` / `decimals_unknown=57`; topology_probe **`edges_built=100`**, `cycles_found=20`, `actual_http_calls=90`, `economics_claim_suppressed=true`, edge_skip=`unknown_token:13` only. Decimals blocker **resolved** for topology_probe; next: symbol/token-map resolution for 13 `unknown_token` routes. **10m quoteability RCA** дозволений у topology_probe; **30m spread-lifetime** лише після `cycles_quoteable>0`. No production/profit claim.
