...
**Cross-Mechanic Sniper Edge (гілка):** Blockers: `M8_2_SPECIALIZED_DEX_MIRROR_RESOLVERS_MISSING` (Curve rolling index не був підключений до hot-path; Balancer/Maverick потребують rolling pool-index + quote smoke) і `M8_2_FRESH_MULTI_VENUE_UNIVERSE_TOO_SMALL`. M8.2 тепер резолвить Curve/Balancer/Maverick через `m8/discovery/mirror_index.py` + rolling artifacts (не generic `ADAPTER_RESOLVE_PENDING`). Hot-path: per-dex `pending_by_dex` / `no_pool_by_dex` / `resolved_by_dex` / `quote_smoke_by_dex` у `m8_hot_path_latest.json`. **15m live WS `--quote`** після `m9_curve_discovery` + index build. M9 bridge-shadow лише при `acceptance.ready_for_bridge_shadow=true`. Strict profit evidence **blocked** until sell-side/tax probe wired.
