...
**Status**: **DYNAMIC_PIPELINE_PARTIAL_REACHED** — fresh M8→bridge→10m topology_probe (`data/tmp/m9_graph_admission_probe_10m_fresh.json`, `m8_stale=False`): `graph_ready_from_m8=63`, `edges_built=406`, topology **364**, sweep `cycles_found=665`, **`cycles_quoteable=132`**, `qsr=0.4928`, `economics_claim_suppressed=true`, `unknown_token=4`. RCA `data/tmp/m9_quote_lane_rca_probe_10m_fresh.json`: `BALANCER_PAUSED=166`, `MAVERICK_NO_LIQUIDITY=98`, `OVERSIZED_VS_DEPTH=389` (non-quoteable). Blockers: **`TOPOLOGY_PROBE_ECONOMICS_SUPPRESSED`**, **`BALANCER_MAVERICK_LEG_REVERTS`**, **`CONFIG_HARDCODE_AUDIT_REQUIRED`** (gate PASS, migration ongoing). Spread-lifetime sidecar: **2 entries** (allowed; `cycles_quoteable>0`). No production/profit claim.
