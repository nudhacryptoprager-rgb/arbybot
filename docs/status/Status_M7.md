...
**Status**: **M7.E1.34o Base 2h soak13 (ended 2026-04-24T21:50:19Z): DATA-LAYER UNBLOCKED / ECONOMICS BLOCKER ACTIVE.** Shipped this cycle: (1) `m7/orderflow/bridge_runtime.py::_rehydrate_hot_unresolved_pools` now issues a single multicall3 `batch_token_info(candidates)` via `core.multicall.get_multicall_batcher()` for all hot-lane unresolved pools, with sequential `eth_call` fallback (selectors `0x0dfe1681/0xd21220a7/0xddca3f43`); `_pool_token_cache` and `pool_token_transport` store the full `(token0, token1, fee_int)` triple instead of `(t0, t1, 0)` — this directly resolves soak9 P0.1 (`HOT_SKIP_UNKNOWN_PAIR`/`TOKEN_ADDRESS_UNKNOWN`). (2) `m7/orderflow/execution_gate.py::_build_sim_tx_params` gained an adaptive-sizing stage (env `ARBY_ADAPTIVE_SIZING=1`, default ON): when `result.best_sweep_size_wei` is smaller than the raw event `amount_in_wei`, sim uses the sweep value; sweep ≥ raw is ignored. (3) +5 unit tests (`test_rehydrate_populates_ptt_on_success` updated, new `test_rehydrate_uses_multicall_fee_tier`, three `test_adaptive_sizing_*`). Runbook: 7/7 supervised processes alive; WS `session_ws_failed_429_windows=0/0`; **PROD** 409 events / 125 fast_scored / **2** positive / **2** sim_attempted / 0 sim_passed (`REVERT:unknown:no_data×2`) / 0 submit_ready / 89 gas_rejected; **DISC** 477 events / 48 fast_scored / **1** positive / **1** sim_attempted / **1** sim_passed / 1 roundtrip_success / **1** submit_ready (paper_signing, first ever on DISC) / `roundtrip_profit_bps_best/median/worst = -9957.9985` / 28 gas_rejected. `TOKEN_ADDRESS_UNKNOWN` per session dropped from soak12 33/23 → **5/5** (~-85%). Interpretation: data layer is no longer the blocker — DISC lane propagates end-to-end for the first time and PROD enters sim for the first time since soak9; however, economic realism now dominates: `roundtrip_profit_bps≈-99.58%` on DISC indicates `best_sweep_size_wei` is still oversized for real pool depth, and PROD sim reverts under `no_data`. Soak14 P0 candidates: (a) profit-realism hard-cap on `amount_in * expected_price_impact / pool_tvl`; (b) calldata/trace dump for `REVERT:unknown:no_data` into `simulation_error_samples`. M7.B remains policy-closed.

Unit baseline: **4259 PASS / 6 skipped / 0 failed** (+4 vs soak9 baseline: 3 adaptive-sizing + 1 new rehydrate-multicall + 1 updated rehydrate-fallback). `check_repo_safety.py`: PASS 0 warnings (all 20 gates).
