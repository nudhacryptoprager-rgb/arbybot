...
**Status**: **M7.E1.34o Base 30m soak15 (ended 2026-04-26T08:35:11Z): BOOTSTRAP+JSON-REFACTOR APPLIED / ECONOMICS GUARD STILL UNVALIDATED.** Soak15 запустився після нового `scripts/bootstrap_system.ps1` (UTF-8 BOM, повна реініціалізація venv/.env/ENV/baselines/cleaner) і `scripts/clean_rolling_artifacts.py` (скоротив rolling JSON: PROD 10677→8504 bytes, DISC 9848→7440 bytes; idempotent, дропає stale `sim_failed_samples`/`sim_output_samples`/пусті histograms). Через зовнішній блокер Tenderly HTTP 403 `slug:insufficient_permissions` (server-side, dashboard permissions; key_len=31, user/proj/key set) PROD перемкнуто на `rpc_fork` backend. Supervisor (6 python procs) завершив clean (0 після deadline). **PROD lane** session 6141956e: 118 events / 16 fast_scored / 0 sim_attempted у session / `session_ws_failed_windows=0` / `session_ws_failed_429_windows=0` / `bridge_hit_but_not_fast_scored.TOKEN_ADDRESS_UNKNOWN=7`. **DISC lane** session 6141956e: 61 events / **0 fast_scored** (`_pool_token_cache` regresія повторюється другий цикл поспіль попри незмінний код) / `TOKEN_ADDRESS_UNKNOWN=8`. Reviewer verdict для обох lanes: **FAIL** (NO_FRESH_SIM_PASSED, NO_FRESH_ROUNDTRIP_ATTEMPTED, FAST_PATH_SCORED_TOO_LOW=16<20 на PROD, STALE_ROLLUP age=196s>120s). Δstrict_provider_breaches=0, ws layer стабільна. M7 production definition (`sim_passed>0 AND submit_ready>0 AND roundtrip_profitable_total>0 AND profit_realism_status=ROUNDTRIP_PROFITABLE`) — **NOT PRODUCTION-READY**. Soak16 plan: (1) патч DISC `_pool_token_cache` у persistent JSON між супервайзер-рестартами, (2) розширити PTT-rehydrate селекторами для pool-types з `TOKEN_ADDRESS_UNKNOWN`, (3) ≥2-год soak або forced/replay reproducer для economics_guard evidence; Tenderly — окремий зовнішній ticket. M7.B remains policy-closed.
