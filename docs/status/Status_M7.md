...
**Status**: **M7.E1.34m fresh Base 30m soak (ended 2026-04-24T06:34:13Z): BLOCKED — root cause now measured; the 20–170s "clean exits" are `recv_error` on the WS peer, not timeouts or structural caps.** Shipped this cycle: (1) `session_id` persisted to `data/runs/_rolling/m7_session_state.json` (TTL 900 s, `ARBY_SESSION_PERSIST=0` to disable) so all 5 lanes now share the SAME `session_id` across every clean-exit restart; (2) `ws_live_stats.exit_reason` + rollup `session_exit_reason_histogram` capture the exact reason the WS loop ended; (3) `/api/summary` + dashboard Monitoring Summary panel render a PERSISTED/ROTATED badge + exit-reason table. Runbook: 5/5 lanes alive 30 min, `crash_restarts=0/100` every subprocess, 69 clean restarts (cycles: m7_hot=22, m7_cold=18, m7_hot_discovery=22, m7_cold_discovery=8). Rollup `session_exit_reason_histogram` PROD `{recv_error: 9}` / DISC `{recv_error: 10}` — **100%** of WS exits are `recv_error`, meaning Alchemy closes the newHeads stream ~50–120 s in, long before `ws_timeout=600` can fire. Deltas vs soak6 baseline: PROD `events_seen_total +14` / DISC `+15`, `fast_path_scored_total +0 / +0`, `sim_attempted/sim_passed/roundtrip_attempted/strict_provider_breaches/BlockOutOfRange +0` on both lanes. Soak8 P0: replace `ws_conn.recv()` implicit-exit with an explicit reconnect loop so the scanner can hold a sustained subscription instead of burning ~65 s cold-starts × 22 cycles. M7.B remains policy-closed.

Unit baseline: **4252 PASS / 6 skipped / 0 failed**. `check_repo_safety.py`: PASS 0 warnings.
