...
**Status**: **M7.E1.40 1.5h soak (E1.26b zero-liq CL refresh): BLOCKED BY ECONOMICS (unchanged root cause). Key fix validated: `registry_has_pair_but_not_pool_count: 0` for all 8 hot cycles (was 9–13). `admitted_to_scoring` improved 1–2 → 13–29/cycle. Fresh soak delta (session 5749662e, 2026-04-29T19:27:47→20:57:54, 5/5 alive, crash_restarts=0): events +184, fast_path_scored +154, sim_passed +3, roundtrip_success_total +2, MISSING_SIZE_METADATA delta=0 (P0 propagation fix CONFIRMED). Bootstrap hardcode fixed: `bootstrap_system.ps1` line 107 previously unconditionally set `ARBY_SIM_MIN_NET_BPS=1.0`, overriding caller's 0.5 → PRE_SIM_SKIP:BELOW_MIN_NET_BPS:1.0 +2 in soak; fixed to `if (-not $env:ARBY_SIM_MIN_NET_BPS) { ... = '1.0' }` (E1.40). Reviewer FAIL only for STALE_ROLLUP (383s>120s, expected post-completion). E1.26b fix: `_refresh_zero_liquidity_entries()` in `m7/orderflow/bridge_runtime.py` re-queries PTT-injected concentrated-liquidity pools that had `sqrt_price_x96>0` but `liquidity=0` at prewarm time (CL positions out of tick range); periodic refresh called each hot iteration in `m7/orderflow/loop_runner.py`. P0 scoring_parallel.py fix (E1.39): `score_backrun_fast()` now propagates `token_in_decimals`, `size_normalization_source`, `size_usd_estimate` to BackrunResult — delta confirms zero new MISSING_SIZE_METADATA this soak. Unit: 4360 PASS / 6 skipped (no regressions). Production-readiness: NOT MET (roundtrip_profitable=0).**
