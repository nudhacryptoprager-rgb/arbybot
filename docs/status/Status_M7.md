...
**Status**: **M7.E1.34 — Sim funnel tightened (admission filter + autofills), anvil backend live on dRPC archive.** Session 38e43e8e (2026-04-21): Base, anvil backend, 64 windows, 0 WS failures, `simulation_backend=anvil` підтверджено у rolling. Cumulative (558 windows) `sim_passed_total=243 / roundtrip_success=228 / roundtrip_profitable_total=16` — runtime plumbing стабільно тримається на dRPC, але pre-Step1/6 error mass (`AMOUNT_ZERO=259`, `VENUE_MISSING=165`) залишається у history histogram і потребує нової 1-год сесії reviewer'ом щоб виміряти дельту. **Новий контракт**: Step 7 (`ARBY_SIM_ADMISSION_STRICT=1` default) відсікає candidates із `net_bps<MIN`, unresolved pair, no-amount/no-fee ДО RPC-виклику → очікується значне зниження навантаження на rate-limited dRPC/Tenderly. `production_readiness` статус незмінний: потрібно `roundtrip_profitable_total>0 AND profit_realism_status=ROUNDTRIP_PROFITABLE` під strict admission.
**Updated**: 2026-04-21

**2026-04-21 note** (session 38e43e8e): Alchemy квота вичерпана → перехід на dRPC Base archive (перевірено getStorageAt на head-100k). Anvil fork стартує на `fork_block=head-5`, приймає eth_call через QuoterV2 (WETH→USDC 0.01 → 23.05 USDC). Session traffic низький (6 fast_scored за 30 хв), тому емпіричної валідації Step 6/7 на rolling artifact ще немає — це відкрите завдання для reviewer (1-hour soak canonical runbook у `docs/DEV_REPORT_LATEST.md`). Identified next blockers: (a) `TOKEN_ADDRESS_UNKNOWN` 152 cumulative → Step 8; (b) anvil block drift — static `fork_block` vs latest → Step 9 (`anvil_mine` periodic loop).
