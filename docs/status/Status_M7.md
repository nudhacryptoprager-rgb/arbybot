...
**Status**: **M7.E1.34n fresh Base 30m soak (ended 2026-04-24T09:33:56Z): BLOCKED — the WS layer is no longer the blocker; token-address resolution has moved into the critical path.** Shipped this cycle: (1) bounded WS reconnect retry loop in `m7/orderflow/mode_ws_live.py` (`ARBY_WS_MAX_RECONNECT_ATTEMPTS=4`, 1.5^n cooldown capped at 8 s, always respecting `ws_timeout`); on success → `reconnected_after_recv_error` + `continue`; on exhaustion → clean `break` with `recv_error_reconnect_failed` — the outer `except Exception` is NEVER entered, so the reviewer's `ws_exception` mislabel disappears. (2) Rollup session aggregates 5 new WS counters: `session_ws_reconnect_total`, `session_ws_recv_error_total`, `session_ws_recv_timeout_total`, `session_ws_subscribe_total`, `last_ws_recv_error`. (3) `bridge_hit_not_scored_reason` classified into explicit buckets (`TOKEN_ADDRESS_UNKNOWN`, `PAIR_FILTER_DROPPED`, `REGISTRY_MISS`, `FAMILY_UNRESOLVED`, `NO_PAIR_MATCH`, …). (4) `_prewarm_registry_from_pairs` publishes `_LAST_PREWARM_STATS` with skipped_missing_tokens / preload_failures / candidates_considered. (5) +2 parametrized reconnect tests. Runbook: 5/5 lanes alive 30 min, `crash_restarts=0/100`, only **8** clean restarts (vs soak7's 69) — one `m7_hot` cycle held **1530 s (25.5 min)** on a single subprocess, structurally impossible before. Session `exit_reason_histogram = {recv_error_reconnect_failed: 23}`, `ws_reconnect/recv_error/subscribe_total = 25/25/25`: bounded retry records exhaustion cleanly but the Alchemy peer still refuses resubscription — root cause moves to soak9. Fast-path blocked: 56 bridge-hit windows attributed to `HOT_SKIP_UNKNOWN_PAIR=38` + `TOKEN_ADDRESS_UNKNOWN=18`; samples show `token_in=None, token_out=None, actual_pair="token0_in/token1"` — discovery lane did not resolve pool tokens in time. Deltas vs pre-soak8 snapshot: PROD `events_seen +77` / DISC `+93`, `fast_path_scored_delta +0 / +0`, `sim_attempted/roundtrip_attempted/strict_provider_breaches/BlockOutOfRange +0`. Soak9 P0: multicall `token0()`/`token1()` inside the prewarm loop before `preload_pair` so hot pools resolve before fast-path consults them. M7.B remains policy-closed.

Unit baseline: **4255 PASS / 6 skipped / 0 failed** (+2 vs soak7). `check_repo_safety.py`: PASS 0 warnings.
