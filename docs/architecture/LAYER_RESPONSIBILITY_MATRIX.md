...
After M8.2 handoff, **all economics-grade token decimals** must come from root aggregator output in:

```text
m8_3_token_metadata_registry_latest.json
  token_registry          # token-level truth (decimals are contract property, not per-DEX)
  dex_route_metadata      # per-route pool/order/indices/hooks (DEX workers → root merge)
```

Child DEX workers collect route/pool metadata only; they **must not** write token decimals or act as quote/economics authority.
...
| DEX route metadata (coin indices, hooks, token order) | M8.3 dex workers → root aggregator |
