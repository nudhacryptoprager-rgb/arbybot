"""
Contract tests for M7.A.4/M7.A.5/M7.A.5.6 — Orderflow-driven replay and live block-event backrun.

Tests lock:
- OrderflowEvent schema and validation
- BackrunResult schema (incl. M7.A.5 live replay fields + M7.A.5.6 coverage/sweep)
...
class TestM7A56BackrunResultFields:
    ...
    def test_field_count_is_42(self):
        ...
        r = score_backrun_offline(ev)
        d = asdict(r)
        assert len(d) == 42, f"Expected 42 fields, got {len(d)}: {sorted(d.keys())}"
...
class TestM7A56AdmitEventTokens:
    ...
    def test_return_schema(self):
        """Return dict must have exactly the documented keys."""
        required_keys = {
...
class TestM7A56BackwardCompat:
    ...
    def test_all_backrun_result_fields_count(self):
        ...
        d = asdict(r)
        # 30 original + 4 M7.A.5.4 + 3 M7.A.5.5 + 5 M7.A.5.6 = 42
        assert len(d) == 42, f"Expected 42 fields, got {len(d)}: {sorted(d.keys())}"
...
class TestM7A55BackwardCompat:
    ...
    def test_total_field_count_42(self):
        """BackrunResult should have exactly 42 fields (30 + 4 M7.A.5.4 + 3 M7.A.5.5 + 5 M7.A.5.6)."""
        r = BackrunResult(
            event_id="fc55",
            event_source="live",
            event_type=EVENT_TYPE_SWAP,
