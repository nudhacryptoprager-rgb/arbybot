...
def test_rejects_total_mismatch(tmp_path: Path):
    scan = tmp_path / "scan.json"
    truth = tmp_path / "truth.json"
    rejects = tmp_path / "rejects.json"

    make_json(scan, {"quotes_total": 5, "current_block": 200})
    make_json(truth, {"quotes_total": 5, "current_block": 200})
    # reject file reports 4 rejects but report will say 2
    make_json(rejects, {"total_rejects": 4, "rejects": [{"r":1},{"r":2},{"r":3},{"r":4}]})
    ...
    assert any("rejects_total_mismatch" in e for e in errs), f"expected rejects_total_mismatch, got {errs}"
