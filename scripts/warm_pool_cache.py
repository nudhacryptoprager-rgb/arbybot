...
v3.2.36: Initial implementation for multi-chain bring-up.
...
DIAGNOSTICS:
    ...
    - zero_liquidity: Pool exists but liquidity=0
...
def get_dex_fee_tiers(chain: str, dex: str) -> List[Optional[int]]:
    ...
    # V3-style DEXes use fee tiers
    v3_dexes = ["uniswap_v3", "pancakeswap_v3", "sushiswap_v3", "lynex_v3", "nuri_v3", "agni_v3"]
    if any(v3 in dex.lower() for v3 in ["_v3", "v3_"]) or dex in v3_dexes:
        return [100, 500, 3000, 10000]  # Standard Uniswap V3 fee tiers
    
    # Algebra DEXes use dynamic fees (fee_tier=0)
    algebra_dexes = ["lynex", "nuri", "algebra"]
    if any(a in dex.lower() for a in algebra_dexes):
        return [0]
    
    # Ve33/Velo-style DEXes use stable/volatile flag
    ve33_dexes = ["aerodrome", "velodrome", "stratum"]
    if any(v in dex.lower() for v in ve33_dexes):
        return [0, 1]  # 0=volatile, 1=stable
    
    # Default: try no fee tier
    return [None]
...
def main():
    ...
    for chain in chains:
        ...
        summary = warm_chain(chain, pairs, args.rpc_url, args.verbose)
