...
M5_0: Centralized constants imported from core/constants
"""
...
ERROR_TO_GATE_CATEGORY = {
    ...
    "PRICE_SANITY_FAIL": "sanity",  # Legacy backward compat
...
def _get_execution_blockers(
    ...
    # M5_0: Use canonical ExecutionBlocker enum values
    if run_mode == RunMode.SMOKE_SIMULATOR.value:
        blockers.append(ExecutionBlocker.SMOKE_MODE_NO_EXECUTION.value)
    
    if execution_disabled:
        blockers.append(ExecutionBlocker.EXECUTION_DISABLED_M4.value)
    
    try:
        pnl = Decimal(str(opp.get("signal_pnl_usdc") or opp.get("gross_pnl_usdc") or opp.get("net_pnl_usdc", "0")))
        if pnl <= 0:
            blockers.append(ExecutionBlocker.NOT_PROFITABLE.value)
    except Exception:
        blockers.append(ExecutionBlocker.INVALID_PNL.value)
    
    try:
        amount_in = opp.get("amount_in_numeraire", "0")
        if Decimal(str(amount_in)) <= 0:
            blockers.append(ExecutionBlocker.INVALID_SIZE.value)
    except:
        pass
    
    confidence = opp.get("confidence", 0.0)
    if confidence < 0.5:
        blockers.append(ExecutionBlocker.LOW_CONFIDENCE.value)
    
    if opp.get("reject_reason"):
        blockers.append(f"REJECTED:{opp.get('reject_reason')}")
    
    if not opp.get("cost_model_available", False):
        blockers.append(ExecutionBlocker.NO_COST_MODEL.value)
...
class TruthReport:
    timestamp: str = ""
    mode: str = "REGISTRY"
    run_mode: str = RunMode.SMOKE_SIMULATOR.value
    execution_enabled: bool = False
    execution_blocker: str = ExecutionBlocker.EXECUTION_DISABLED_M4.value
...
def build_truth_report(
    ...
    # M5_0: Use canonical ExecutionBlocker
    execution_blocker = ExecutionBlocker.EXECUTION_DISABLED_M4.value
    if run_mode == RunMode.SMOKE_SIMULATOR.value:
        execution_blocker = ExecutionBlocker.SMOKE_MODE_NO_EXECUTION.value
